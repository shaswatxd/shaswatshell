...
// Orbiting Cyber Particle Swarm
function ParticleSwarm({ count = 250 }) {
  ...
  const [positions, colors] = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      // Sphere distribution radius 1.4 - 2.5
      const u = Math.random();
      const v = Math.random();
...
function CyberCore({ mousePos }) {
  ...
  useFrame((state, delta) => {
    if (groupRef.current) {
      ...
      groupRef.current.rotation.x += (-mousePos.current.y * 0.4 - groupRef.current.rotation.x) * 0.05;
    }
    ...
    if (ring1Ref.current) {
      ring1Ref.current.rotation.x = Math.PI / 3 + state.clock.getElapsedTime() * 0.3;
      ring1Ref.current.rotation.y = state.clock.getElapsedTime() * 0.2;
    }

    if (ring2Ref.current) {
      ring2Ref.current.rotation.x = -Math.PI / 4 + state.clock.getElapsedTime() * 0.25;
      ring2Ref.current.rotation.z = state.clock.getElapsedTime() * 0.35;
    }
  });
    ...
    <Float speed={2.5} rotationIntensity={0.4} floatIntensity={0.6}>
      <group ref={groupRef}>
        ...
        <mesh scale={0.35}>
          ...
          <meshBasicMaterial color="#00c2d1" transparent opacity={0.6} />
        </mesh>

        {/* Orbiting Gyros Ring 1 */}
        <group ref={ring1Ref}>
          <Ring args={[1.6, 1.63, 64]}>
            <meshBasicMaterial color="#8b6bff" side={THREE.DoubleSide} transparent opacity={0.5} />
          </Ring>
        </group>
        ...
        <group ref={ring2Ref}>
          <Ring args={[1.8, 1.82, 64]}>
            <meshBasicMaterial color="#00c2d1" side={THREE.DoubleSide} transparent opacity={0.4} />
          </Ring>
        </group>

        {/* Particle Stars */}
        <ParticleSwarm count={220} />
      </group>
    </Float>
  );
}
...
export default function Hero3D() {
  ...
  return (
    <div className="relative w-full h-[260px] sm:h-[400px] lg:h-[480px] flex items-center justify-center pointer-events-none select-none">
      <Canvas
        dpr={1}
